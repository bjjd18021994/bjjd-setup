...
if helm list -n data | grep -q "bjjd-postgres"; then
    echo -e "${YELLOW}Found release 'bjjd-postgres'. Uninstalling...${NC}"
    helm uninstall bjjd-postgres --namespace data || true
else
...
kubectl delete job postgres-init-databases --namespace data --ignore-not-found >/dev/null 2>&1 || true

echo -e "${YELLOW}Deleting PVC 'pg-pvc' if it exists...${NC}"
kubectl delete pvc pg-pvc --namespace data --ignore-not-found >/dev/null 2>&1 || true
...
kubectl delete secret postgres-db-secrets --namespace data --ignore-not-found >/dev/null 2>&1 || true

echo -e "${YELLOW}Deleting ConfigMap 'postgres-config' if it exists...${NC}"
kubectl delete configmap postgres-config --namespace data --ignore-not-found >/dev/null 2>&1 || true

echo -e "${CYAN}Applying PV, PVC, ConfigMap, and Secrets...${NC}"
kubectl apply -f "postgres-setup-files/postgres-pv.yaml" --namespace data
kubectl apply -f "postgres-setup-files/postgres-pvc.yaml" --namespace data
kubectl apply -f "postgres-setup-files/postgres-secret.yaml" --namespace data
kubectl apply -f "postgres-setup-files/postgres-config.yaml" --namespace data
...
kubectl apply -f "postgres-setup-files/db-init-job.yaml" --namespace data
